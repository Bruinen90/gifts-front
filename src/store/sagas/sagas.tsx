...
function* createDraw(action: { type: string; payload: DrawInterface }) {
    const { title, date, price, creatorsId, participantsIds } = action.payload;
    const graphqlQuery = {
        query: `
			mutation CreateNewDraw($title: String!, $date: String!, $price: Int!, $creatorsID: ID!, $participantsIDs: [ID!]!) { 
